...
fs.readdir(INPUT, (errFolder, files) => {
  ...
  files
    .filter(
      file =>
        path.extname(file).toLowerCase() === ".json" && file !== "sample.json"
    )
    .forEach(file => {
      fs.readFile(
        `${INPUT}/${file}`,
        ...
        (errFile, content) => {
          ...
          fs.writeFile(outputFile, vcal, errWrite => {
            ...
            ssh
              .connect(({ host, username, password, port } = deploy))
              .then(() => {
                ssh
                  .putFile(outputFile, deploy.location)
                  .then(() => {
                    console.log(
                      `${outputFile} deployed to ${deploy.location}!`
                    );
                    ssh.dispose();
                  })
                  .catch(err => {
